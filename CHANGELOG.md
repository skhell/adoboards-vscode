...
### Fixed
- **Source Control diff** - `parent` field no longer appears as a new addition in diffs; `reconstructMarkdown()` now includes `parent` from the ref entry
- **File decoration** - Modified files now highlight immediately on save via `onDidSaveTextDocument` (faster than OS file watcher on macOS)

### Removed
- **Sidebar panel** - Removed redundant Activity Bar icon and Work Items tree view; Source Control panel is the single source of truth
  - Removed `viewsContainers`, `views`, `viewsWelcome` contributions
  - Removed `onView:adoboards.panel` activation event
  - Removed `WorkItemTreeProvider` and all tree-related command handling

## [0.1.1] - 2026-03-13

### Changed
- Source Control welcome message now directs users to the SCM panel instead of the sidebar tree view
